...
//			if( ch >= 'a' && ch <= 'z')
//			result += Character.toString(abcCode[ch-97]);
//			if( ch >= '0' && ch <= '9')
//			result += Character.toString(numCode[ch-49]);
//		 	}
...
//		ystem.out.println();
		...
		int[][] result = new int[score.length+1][score[0].length+1]; // result 6행 4열
			for(int i=0; i < score.length;i++) {
				for(int j=0; j < score[i].length;j++) {
